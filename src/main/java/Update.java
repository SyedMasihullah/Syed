...
public class Update extends HttpServlet {
	...
	protected void cMobile(HttpServletRequest request, HttpServletResponse response)
	{
		...
		try {
			...
			PreparedStatement ps =myconnection.prepareStatement("UPDATE mobiles SET Product = ?, Brand = ?, price=? WHERE Proid = ?");
			...
			if (r>0) {
				response.getWriter().print(proid+product+"  has been changed");
			}
		} catch (Exception e) {
			e.printStackTrace();
		}
